...
public class LocalDatabaseHelper extends SQLiteOpenHelper {
    ...
    public static List<Style>getStyles(String needtype,SQLiteDatabase db){
        ...
        Cursor cursor;
        if(needtype.equals(ConstValue.type_make_all)){
            cursor = db.rawQuery("select * from styles ", null);
        }else{
            cursor = db.rawQuery("select * from styles where type=?", new String[]{needtype});
        }
    ...
    public static List<Pic>getUserPic(String needaccount,SQLiteDatabase db){
        ...
        Cursor cursor = db.rawQuery("select * from userfiles where account="+needaccount+" order by id desc", null);
        if(cursor.moveToFirst()){
            do{
                int id=cursor.getInt(cursor.getColumnIndex("id"));
