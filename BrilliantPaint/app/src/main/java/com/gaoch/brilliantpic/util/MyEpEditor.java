...
public class MyEpEditor {
    ...
    public static void exec(EpVideo epVideo, MyEpEditor.OutputOption outputOption, OnEditorListener onEditorListener) {
        ...
        if (epDraws.size() > 0) {
            ...
            filter_complex.append("[0:v]").append(epVideo.getFilters() != null ? epVideo.getFilters() + "," : "").append("scale=").append(outputOption.width == 0 ? "iw" : outputOption.width).append(":").append(outputOption.height == 0 ? "ih" : outputOption.height).append(outputOption.width == 0 ? "" : ",setdar=" + outputOption.getSar()).append("[outv0];");
    ...
    public static void merge(List<EpVideo> epVideos, MyEpEditor.OutputOption outputOption, OnEditorListener onEditorListener) {
    ...
    public static void mergeByLc(Context context, List<EpVideo> epVideos, MyEpEditor.OutputOption outputOption, OnEditorListener onEditorListener) {
        String appDir = context.getFilesDir().getAbsolutePath() + "/EpVideos/";
        String fileName = "ffmpeg_concat.txt";
        List<String> videos = new ArrayList();
        Iterator var7 = epVideos.iterator();

        while(var7.hasNext()) {
            EpVideo e = (EpVideo)var7.next();
    ...
    public static void demuxer(String videoin, String out, MyEpEditor.Format format, OnEditorListener onEditorListener) {
        CmdList cmd = new CmdList();
        cmd.append("ffmpeg").append("-y").append("-i").append(videoin);
        switch(format) {
            case MP3:
    ...
    public static void changePTS(String videoin, String out, float times, MyEpEditor.PTS pts, OnEditorListener onEditorListener) {
        if (times >= 0.25F && times <= 4.0F) {
            ...
            cmd.append("-preset").append("superfast").append(out);
            long d = VideoUitls.getDuration(videoin);
            double dd = (double)((float)d / times);
            long ddd = (long)dd;
